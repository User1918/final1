...
PARAM_PATH = "/home/labthayphuc/Desktop/yolov11ncnn/yolov11n_int8.param" # <<< UPDATE THIS PATH
BIN_PATH = "/home/labthayphuc/Desktop/yolov11ncnn/yolov11n_int8.bin"     # <<< UPDATE THIS PATH
...
DEFAULT_MAX_SPEED = 120        # Default speed limit if no sign detected (used for reset)
MANUAL_MAX_SPEED = 100           # Absolute max speed achievable with 'w' key (caps requested speed)
...
def warpImg():
    ...
    points = np.float32([(leftTop, heightTop), (640 - rightTop, heightTop),(leftBottom, heightBottom ), (640 - rightBottom, heightBottom)])
    matrix = cv2.getPerspectiveTransform(points, np.float32([[0, 0], [w, 0], [0, h], [w, h]]))
    ...
    lines = cv2.HoughLinesP(imgWarp, 1, np.pi/180, threshold=100, minLineLength=100, maxLineGap=0)
...
def SteeringAngle():
    ...
    for i in range(im_height - 1, set_point, step):
        ...
        if left_point != -1 and right_point != -1:
            ...
            if lane_width > lane_width_max:
                lane_width_max = lane_width
            if lane_width_max > 550:
                lane_width_max = 550
        if left_point != -1 and right_point == -1:
...
#         current_speed_limit = DEFAULT_MAX_SPEED
# 7. Determine Final Commanded Spee
            ...
            last_speed = transform_speed(last_speed_sent)
            ...
            # Print status less frequently or conditionally to avoid spammin
            if frame_counter % 10 == 0: # Print every 10 frames
                ...
                center_img = im_width // 2
                cv2.line(imgWarp, (center_img, im_height), (center_img, im_height - 50), (0, 0, 255), 1) # Center line marker red
                if left_point != -1 or right_point != -1:
                     ...
                     cv2.line(imgWarp, (mid_point_viz, im_height), (mid_point_viz, im_height - 50), (0, 255, 0), 1) # Mid point marker green
                # Add text overlays
                cv2.putText(imgWarp, f"FPS: {instant_fps:.1f}", (imgWarp.shape[1] - 120, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1)
                cv2.putText(imgWarp, f"CTE: {cte_f:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
                cv2.putText(imgWarp, f"Steer: {int(round(steering))}", (10, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
                cv2.putText(imgWarp, f"Speed Cmd: {last_speed_sent}", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
                cv2.putText(imgWarp, f"Limit: {current_speed_limit}", (10, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
                if stop_condition_met: 
                    cv2.putText(imgWarp, "STOP", (imgWarp.shape[1] // 2 - 40 , imgWarp.shape[0] // 2), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
                # Display detected signs on the ROI image
                sign_y_offset = 130
                for sign in detected_signs:
                     sign_text = f"{sign['name']} ({sign['confidence']:.2f})"
                     cv2.putText(imgWarp, sign_text, (10, sign_y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
                     sign_y_offset += 20

                cv2.imshow("Lane Detection ROI", imgWarp)
