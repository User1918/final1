...
def detect_signs_and_get_results(input_frame_bgr):
    ...
    if len(output_data.shape) != 2: print("Unexpected output shape"); return detections_results
    ...
    if boxes:
        dummy_boxes_for_nms = [[0, 0, 1, 1]] * len(confidences)
...
def transform_speed(velocity):
    """ Converts target velocity (km/h) to estimated motor RPM. """
    # ... (Keep transform_speed function exactly as it was, REMEMBER TO ADJUST CONSTANTS) ...
    WHEEL_DIAMETER_M = 0.0475 * 2 # Example: 47.5mm radius * 2
    GEAR_RATIO = 2.85 * 30         # Example: Motor to wheel gear reduction
    KMH_TO_MPS = 1000 / 3600
    if WHEEL_DIAMETER_M <= 0: return 0
    velocity_mps = velocity * KMH_TO_MPS
    wheel_circumference_m = np.pi * WHEEL_DIAMETER_M
    wheel_rps = velocity_mps / wheel_circumference_m
...
# --- Main Execution ---
def main():
    global steering, speed_motor_requested, flag, current_speed_limit, net, ser, frame_counter, loop_start_time, overall_fps # Ensure globals are accessible
        ...
        while True:
            ...
            if detected_signs:
                for sign in detected_signs:
                    if sign['name'] in ["Stop Sign", "Traffic Light -Red-"]:
            ...
            if ser:
                try:
                    ...
                    ser.flush()
                    last_sent_rpm = final_speed_command_rpm
                    last_sent_steer = steering
                except serial.SerialException as e:
            ...
            if imgWarp is not None:
                ...
                cv2.putText(display_img, f"Speed Cmd: {final_speed_command_rpm} RPM", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
                cv2.putText(display_img, f"(Req: {speed_motor_requested} vel)", (10, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 0), 1)
