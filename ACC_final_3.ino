...
float Kp = 1.5;  // Example starting value - TUNE ME!
...
float updateEMA(float currentRawValue) {
 ...
 //=========================================================================
 // MESURE DISTANCE && CONTROL SPEED FOLLOW DISTANCE
 //=========================================================================
float measureDistanceCM() 
{
  ...
  digitalWrite(trigPin, LOW);
  duration = pulseIn(echoPin, HIGH, 5000); // 3.5ms ï¿½ 60cm timeout

  if (duration == 0) {
    // No echo received ï¿½ return -1 to signal invalid reading
    return -1.0;
  }
...
float adjustRPMWithDistance(float distance) {
    static float lastRPM = 0.0;

    if (distance < 10.0) {
         RPM_setpoint = 0.0; // STOP motor
    } 
    else if (distance >= 10.0 && distance <= 35.0) 
    {
        // ---  deadband giu nguyen toc do toi khoang 30cm ---
        if (distance >= 29.0 && distance <= 31.0)
        {
            return lastRPM; // Khong thay doi RPM
    }

        float RPM_calculate = ((Max_distance - distance) * (RPM_setpoint / Max_distance));
        lastRPM = RPM_setpoint - RPM_calculate;
    } 
    else if (distance > 35.0 && distance < Max_distance) {
        // --- Tang den RPM ve RPM_setpoint ---
        lastRPM += 5.0; // Tang t? t?
        if (lastRPM > RPM_setpoint) lastRPM = RPM_setpoint;
    } 
    else {
        lastRPM = RPM_setpoint; // KEEP SPEED TO RPM_setpoint.
    }
...
void displayData() {
    ...
    if (millis() - lastPrintTime >= PRINT_INTERVAL_MS) {
        ...
        Serial.print(g_filteredRPM,6); // <<< Print the global filtered value
        Serial.print(",");
...
void loop() {
 ...
 // Get the latest raw RPM calculated by the timer callbac
    float rawRPM;
    noInterrupts();
    rawRPM = currentRPM;
    interrupts();
    ...
    if (measured >= 0.0) {
    distanceCm = measured; // update only if valid
    }
    ...
    if (currentMillis - lastPIDRunTime >= PID_INTERVAL_MS) 
    {
        ...
        float targetEffort_RPM = calculatePID_RPM_Output(adjustedSetpoint, g_filteredRPM); // <<< Use g_filteredRPM here
